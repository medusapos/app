...
it('keeps refused sales pending, explains the refusal, and sends them when Try again succeeds', async () => {
  ...
  const strip = screen.getByText('1 sale saved, the store is refusing them ·').parentElement!.parentElement!;
  ...
  expect(screen.getByRole('heading', { name: 'The store is refusing these sales' })).toBeTruthy();
...
it('uses plural sales copy for a refused batch', async () => {
  ...
  expect(screen.getByText('2 sales saved, the store is refusing them ·')).toBeTruthy();
  fireEvent.click(screen.getByRole('button', { name: 'Details' }));
