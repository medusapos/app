...
export function OutboxStrip({ children, header }: { children: ReactNode; header?: ComponentProps<typeof SignInAgain>['header'] }) {
  ...
  const onLayout: ComponentProps<typeof View>['onLayout'] = (event) => setHeight(event.nativeEvent.layout.height);
  return <StripHeightContext.Provider value={!header && session && (state.authRequired || state.refused) ? height : 0}>
    <View style={{ flex: 1 }}>
      {children}
      {state.authRequired || !state.refused ? <SignInAgain header={header} onLayout={onLayout} />
        : <StoreRefused header={header} onLayout={onLayout} />}
    </View>
  </StripHeightContext.Provider>;
}

export function StoreRefused({ header, onLayout }: ComponentProps<typeof SignInAgain>) {
  ...
  const strip = useMemo(() => <View dataSet={{ print: 'hide' }} className="flex-row items-center gap-1">
    <Text numberOfLines={1} className="shrink text-foreground">{state.pending} {state.pending === 1 ? 'sale' : 'sales'} saved, the store is refusing them ·</Text>
  ...
  return <View onLayout={onLayout} dataSet={{ print: 'hide' }} style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 1 }}
    ...
    {!expanded ? strip : <>
      <Text accessibilityRole="header" className="font-semibold text-foreground">The store is refusing these sales</Text>
